...
class MintNFTModal(discord.ui.Modal, title="Mint NFT"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        button = discord.ui.Button(label=f"➡️ Click to mint NFT", style=discord.ButtonStyle.danger, url=url)
        view = View()
        view.add_item(button)
...
async def wallet(interaction: discord.Interaction):
    ...
    async def tbd_callback(interaction: discord.Interaction):
        await interaction.response.send_message("You are fine as you are. Learn to accept yourself. And Stay COTI!", ephemeral=True)
    ...
    embed.add_field(name="This proof-of-concept is built on COTI testnet", value="", inline=True)
